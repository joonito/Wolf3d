...
/*   Updated: 2020/02/09 19:50:04 by julee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_forward(t_wf3d *wf3d, t_pointf d_unit, t_pointf c_unit)
{
	...
	&& (int)(wf3d->my_pos.y + d_unit.y) >= 0
	&& (int)(wf3d->my_pos.x + d_unit.x) < (int)wf3d->map_width
	&& (int)(wf3d->my_pos.x + d_unit.x) >= 0
	&& (wf3d->map[(int)(wf3d->my_pos.y + d_unit.y)][(int)(wf3d->my_pos.x \
			+ d_unit.x)] == 0))
	{
...
void	move_leftside(t_wf3d *wf3d, t_pointf d_unit, t_pointf c_unit)
{
	...
	&& (int)(wf3d->my_pos.y - c_unit.y) >= 0
	&& (int)(wf3d->my_pos.x - c_unit.x) < (int)wf3d->map_width
	&& (int)(wf3d->my_pos.x - c_unit.x) >= 0
	&& (wf3d->map[(int)(wf3d->my_pos.y - c_unit.y)][(int)(wf3d->my_pos.x \
			- c_unit.x)] == 0))
	{
...
void	key_press_move(t_wf3d *wf3d, int keycode)
{
	...
	c_unit = to_unit_vec(wf3d->camera_plane);
